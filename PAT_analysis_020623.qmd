...
data <- rjson::fromJSON(file = "patdeployments-default-rtdb-murphyphdstudent1-export.json")
data <- purrr::map(data, ~ purrr::keep(.x, .p = stringr::str_detect(names(.x), regex("\\b(?i)[A][1-9]{3}\\b")))) %>%
  purrr::compact()
...
data_clean <- list_flatten(data) %>%
  ...
  dplyr::rename(bl = baselines,
         st = syncroTraining) %>%
  tidyr::unnest_longer(bl) %>%
  tidyr::unnest_wider(bl, names_sep = "_") %>%
  tidyr::unnest_longer(st) %>%
  tidyr::unnest_wider(st, names_sep = "_") %>%
  tidyr::drop_na(st_averagePeriods, st_currentDelays) %>%
  dplyr::group_by(participantID) %>%
  dplyr::mutate(st_trial = row_number()) %>%
  dplyr::filter(st_trial > 2) %>%
  dplyr::group_by(participantID) %>%
  dplyr::mutate(st_trial = row_number())
...
data_clean <- data_clean %>% dplyr::filter(lengths(st_averagePeriods) > 0 &
...
data_clean <- data_clean %>% 
  ...
  dplyr::filter(st_trial_comp >= st_trials_needed) %>%
  dplyr::group_by(participantID) %>%
  dplyr::slice(1:st_trials_needed) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(st_last_delay = st_currentDelays[length(st_currentDelays)],
  ...
  dplyr::mutate(sim_overall = calc_similarity(st_last_delay, st_last_period),
...
hr <- data_clean %>%
  dplyr::group_by(participantID) %>%
  dplyr::select(participantID, bl_ibi) %>%
...
# summary_bayes <- summary %>% dplyr::filter(intero_bayes_3 != "Unknown")
...
# summary_bayes_10 <- summary %>% dplyr::filter(intero_bayes_10 != "Unknown")
...
## remove individuals with Unknown final label (unvomment to remove)
# summary_bayes_30 <- summary %>% dplyr::filter(intero_bayes_30 != "Unknown")
...
sim_HRV <- summary_bayes_30 %>% dplyr::dplyr::select(similarity, mean_HR, hrv_sdnn, hrv_rmssd, hrv_pnn50)
...
sim_HRV_cor<- sim_HRV_cor %>%
  dplyr::dplyr::select(-c(statistic, conf.low,conf.high)) %>% 
  arrange(desc(var2))
...
int_HRV <- summary_bayes_30 %>%
  dplyr::filter(intero_bayes_3 =="Interoceptive participants") %>%
  dplyr::dplyr::select(similarity, mean_HR, hrv_sdnn, hrv_rmssd, hrv_pnn50)
...
int_HRV_cor<- int_HRV_cor %>% dplyr::dplyr::select(-c(statistic, conf.low,conf.high)) %>% arrange(desc(var2))
...
non_HRV <- summary_bayes_30 %>% dplyr::filter(intero_bayes_3 =="Non interoceptive participants") %>%
  dplyr::dplyr::select(similarity, mean_HR, hrv_sdnn, hrv_rmssd, hrv_pnn50)
...
non_HRV_cor<- non_HRV_cor %>% dplyr::dplyr::select(-c(statistic, conf.low,conf.high)) %>% arrange(desc(var2))
...
sim_engage <- summary_bayes_30 %>% dplyr::dplyr::select(similarity, mean_time_trials, mean_engagement_trials )
...
sim_engage_cor <- sim_engage_cor %>% dplyr::dplyr::select(-c(statistic, conf.low, conf.high)) %>% arrange(desc(var2))
...
## Correlation in interoceptive participants
int_engage <- summary_bayes_30 %>% dplyr::filter(intero_bayes_3 =="Interoceptive participants") %>%
  dplyr::dplyr::select(similarity, "mean_time_trials", "mean_engagement_trials")
...
int_engage_cor <- int_engage_cor %>% dplyr::dplyr::select(-c(statistic, conf.low,conf.high)) %>% arrange(desc(var2))
...
### Correlation in Non-interoceptive participants
non_engage <- summary_bayes_30 %>% dplyr::filter(intero_bayes_3 =="Non interoceptive participants") %>%
  dplyr::dplyr::select(similarity,"mean_time_trials", "mean_engagement_trials")
...
non_engage_cor<- non_engage_cor %>% dplyr::dplyr::select(-c(statistic, conf.low,conf.high)) %>% arrange(desc(var2))
